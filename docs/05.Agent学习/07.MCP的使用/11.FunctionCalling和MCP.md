---
title: FunctionCalling和MCP
date: 2026-03-19 14:05:54
permalink: /pages/d3d5c9/
categories:
  - HelloAgent
  - 理论扩展
tags:
...
MCP 规定了服务器如何发现函数，如何调用函数

MCP本身，并不会和模型有直接关系 

### Function Calling

Function Calling 并不是函数调用，**真正调用函数，并获取返回结果的是MCP的工作**

Function Calling 的作用是给大模型函数的**使用方式、参数、以及如何调用工具列表中的工具**
...
![](./FunctionCalling&McpImg/searchWeatherProcess.png)
...
![](./FunctionCalling&McpImg/promptFuncCallingProcess.png)
...
![](./FunctionCalling&McpImg/ApiFuncCallingProcess.png)
...
![](./FunctionCalling&McpImg/MCPSystemArch.png)
...
![img](./FunctionCalling&McpImg/MCPartc.png)
...
![](./FunctionCalling&McpImg/MCPHost.png)

![](./FunctionCalling&McpImg/MCPInstance.png)
...
2. **工具与AI 应用之间的交互必须标准化。**
   ...
   2. AI 应用和工具服务的**接口定义**需要统一（需要提供哪些接口、接口需包含哪些参数）
      ...
      4. ![](./FunctionCalling&McpImg/requestToolList.png)

      5. ![](./FunctionCalling&McpImg/responseToolList.png)
...
![](./FunctionCalling&McpImg/MCPFuncCallingProcess.png)

### MCP 与Function calling

> **值得一提的是，<u>MCP 协议和 Function Calling</u>之间绝不是“技术递进”的关系。所谓“MCP 协议会取代 Function Calling”的说法，其实是一种不严谨的表达。**

1. Function calling 主要是 AI 应用程序和大模型交互的这部分，也就是上图中，2.2~2.3的部分，拿一个工具列表，返回一个文字版的调用指令

2. MCP 协议，关心的是AI应用程序，怎么和MCP server进行交互的这部分，也就是怎么拿到function calling 过程中所需要的函数描述，也就是3.1~3.4，如何根据function calling返回的工具指令，去实现真正的工具调用

3. function calling 更倾向于一种能力，MCP协议更倾向于一种协议和规范，一种技术上的共同约定
