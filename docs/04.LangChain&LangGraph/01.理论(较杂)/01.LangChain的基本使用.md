...
1. 模型(Models)：
   ...
   1.  **提示模板**：管理和复用提示词。
   2. **模型包装器**：统一调用OpenAI、Anthropic、文心一言等模型。
   3. **输出解析器**：将模型输出结构化为JSON等格式。

2. 记忆(Memory)：

   在对话或多次调用中维护和传递状态，让应用具备上下文“记忆”
...
4. 链(Chain)：
   ...
   将多个组件按顺序组合成一个可执行的、复杂的任务流程

5. 智能体(Agent)：

   由LLM驱动的决策系统，能根据用户指令和当前状态，自主选择并调用工具（工具）来完成复杂任务

6. 回调(Callback)：

   提供一套事件处理系统，用于**日志记录、监控、调试**和流式处理，贯穿所有模块
...
- **智能体（Agents）**：允许模型自主调用外部工具和组件，实现多步骤任务处理，如AutoGPT和BabyAGI。
- **记忆（Memory）**：存储和检索对话数据，支持上下文感知的应用，如多轮对话系统。
- **检索（Indexes）**：组织和检索文档数据，支持RAG（检索增强生成）等应用场景。

![](./img/langChainFunc.png)

## langChain包的分类

1. langchain： 核心包
2. langchain-community：社区包，提供了更多的第三方模型调用(我们用的阿里云千问模型，就需要这个包)
3. langchain-ollama：Ollama支持包，支持调用Ollama托管部署的本地模型
4. dashscope： 阿里云通义千问的Python SDK
5. chromadb：轻量向量数据库(后续使用)
