---
title: 接入OpenAI_Agents_SDK
date: 2026-05-08 12:42:42
permalink: /pages/d0391b/
categories:
  - 项目总结
  - ITS多智能体
tags:
...
- Python 3.10+ / 虚拟环境
- 安装：`pip install openai` `pip install openai-agents` 
- 配置：`OPENAI_API_KEY=...
...
| **工具调用**   | 需在请求中定义`tools`参数，并手动处理模型的工具调用请求和返回结果，流程复杂。 | **原生、自动化程度高**。直接在请求中提供`tools`，模型可自主决定何时、如何调用，并返回最终结果（内置工具）。 |

简单来说，**Chat Completions API 是“对话模式”，而 Responses API 是“任务模式”**。 是一个**“自带大脑和记忆”的任务务执行器**，它不再只是简单地“接话”，而是能作为真正的 **Agent** 去自主完成复杂的、多步骤的工作。后者专为构建更智能的AI应用（即智能体）而设计。
...
使用**Responses API** 携带工具（agent自带的工具）
...
| 说明                               | 方式                                            | 作用范围                    | 适用场景                  |
| ---------------------------------- | ----------------------------------------------- | --------------------------- | ------------------------- |
| 所有模型，都使用同一个客户端       | **方式一`set_default_openai_client`：全局设置** | 所有 Agent 共享同一个客户端 | 单模型项目                |
| 每次运行时，决定使用哪个模型       | **方式二`ModelProvider`：运行时传入**           | 单次 Run 指定客户端         | 多模型混合调度            |
| 和agent绑定，该agent一直使用该模型 | **方式三`Agent.model`：绑定到单个 Agent**       | 仅该 Agent 使用指定客户端   | 精细控制每个 Agent 的模型 |
...
> 优点：Agent 自包含，不依赖外部配置，这是构建复杂多 Agent 系统的最佳方式。
...
等。这里面我们以两个核心状态为例。Streaming 状态、历史消息（Conversation）
