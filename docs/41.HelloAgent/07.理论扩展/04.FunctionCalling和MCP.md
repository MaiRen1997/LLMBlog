...
![](./img/searchWeatherProcess.png)

#### **存在的问题**

1. **是否调用工具、调用什么工具由后端负责判断，逻辑复杂且容易误判**。

   1. 比如用户输入，要查询杭州余杭的天气，结果正则匹配，查询了杭州市的天气，就会出现问题
...
##### **广义的 Function Calling** 

1. **是指让大模型能够调用外部工具的一种技术实现**：先向大模型提供可用函数的列表及说明，由大模型在对话过程中智能判断是否需要调用函数，并自动生成调用所需的参数，最终**用文字返回符合约定格式的函数调用请求**。
2. 比如给大模型一段话，让其按照特定的json进行解析输出，然后根据json里的参数，进行方法调用

##### **狭义的 Function Calling**

 特指大模型提供商在模型内部与 API 层面做了支持的一种能力，它最早由 OpenAI 引入：
...
`实际过程中，基于提示词的Function calling 更常见，`

`因为不同模型可能有不同的API，标准很难统一，所以基于API的Function calling 没有得到广泛的应用`

#### 基于提示词的 Function Calling

##### 工作流程

![](./img/promptFuncCallingProcess.png)
...
1. 后端应用适配不同大模型时存在大量冗余开发
   ...
   (不同的大模型开发商，要求不同的输入格式，也会传回不同的输出格式)
...
2. **工具复用困难**
   ...
   1. 1.  环境问题导致 **copy** 的代码不一定能跑；
      2. 很多企业不提供可供 **copy** 的源码；
      3. 跨语言的代码 **copy** 了没用。
...
- 思路一：**导包式接入**，AI 应用开发者将工具代码拉到本地调用。
  ...
  - 可行吗？⚠️ **跨语言调用**问题无法解决，可以用下面的方法解决：  

  - 跨语言问题是客观无解的吗？->

     能否本地另起一个进程运行该语言的执行环境，工具代码原封不动运行在对应语言环境中，

    AI 应用再通过进程间通信（如匿名管道、有名管道、消息队列、共享内存信号(信号量)、套接字）

    获取标准化的返回结果

  - 如果企业不愿共享源码，可以打包成exe，然后AI根据操作系统和功能，判断需要执行哪个exe

    -> 并且，这种接入方式更适合被称为“**本地服务式接入**”
...
- **MCP Client**：一个组件，用于维护与 MCP 服务器的连接，并从 MCP 服务 器获取上下文，供 MCP 主机使用
- **MCP Server**：一个为 MCP Client 提供上下文的程序(<u>就是上图中，包含许多工具的一个server</u>)

例如：Visual Studio Code 充当 MCP 主机。当 Visual Studio Code 建立与MCP服务器（如[Sentry MCP服务器](https://docs.sentry.io/product/sentry-mcp/)）的连接时，Visual Studio Code 运行时实例化了维护与Sentry MCP服务器连接的MCP客户端对象。

当Visual Studio Code 随后连接到另一个MCP服务器时，例如[本地文件系统服务器](https://github.com/modelcontextprotocol/servers/tree/main/src/filesystem)，Visual Studio Code 运行时实例化一个额外的MCP客户端对象来维护此连接，从而保持MCP客户端与MCP服务器的一对一关系。

![img](https://oigi8odzc5w.feishu.cn/space/api/box/stream/download/asynccode/?code=YjRjZDg1ZGI5YjNiMGE5NTgzMjFmYTU3NTU2ODc1YjZfNm5remVIclJBclVkQ0YyRVhkSGhpM1pHaDJLaHY2aXBfVG9rZW46SnN2T2JHME9Wb0FCanp4UXp2RWNDeU16bmtmXzE3NzM5MDAzMzg6MTc3MzkwMzkzOF9WNA)
...
1. 什么是 **stdio**？
   ...
   1. **stdio**（standard I/O）是**进程的标准输入/输出接口。**

      每个进程启动时，操作系统会给它分配 **三个文件描述符**：
...
2. 什么是**管道（pipe）**？
   ...
   1. **管道**是操作系统内核提供的一种**进程间通信（IPC）机制**，

      它允许一个进程的输出直接作为另一个进程的输入，实现数据在两个进程之间的流动。

3. 总结：什么是 Stdio 传输 ？

   1. 所谓 Stdio 传输，就是通过**标准输入**和**标准输出**这两个数据流来传输数据（这两个只是**接口**，中间**通过管道进行连接**）、

      通过管道来连接两个进程的**标准输入/输出接口**，使得一个进程的输出直接传给另一个进程输入，实现进程间数据传输（本质上是一个基于字节流的全双工通信通道）
...
- 基本原理
  ...
  2. 服务端保持这个连接**不断开**，以 `text/event-stream` 作为响应类型，源源不断地往里写数据。

     - 之所以用SSE，而不用websocket，虽说两者都可以由服务端发起请求，

       但是因为**SSE可以流式的传输，而ws,只是一次输出**

  3. 客户端收到数据后会触发相应的事件回调（比如浏览器前端实时更新界面）。
...
1. **数据格式限制问题**：
   - SSE 的 `Content-Type: text/event-stream` 只支持文本格式；
   - Streamable HTTP 的`Content-Type`支持任意格式，如 JSON、HTML、二进制等，**更适合 AI 场景**（可能要传 JSON + 音频 + 图片）
2. **跨平台兼容问题**：
   - SSE 支持的客户端主要是浏览器端和少量语言库；
   - 而 Streamable HTTP 支持多种客户端。
3. **性能问题：**
   - SSE 是基于 **HTTP/1.1** 长连接，
   - Streamable HTTP 可以基于 **HTTP/2/3** ，支持**多路复用和双向流**。
   - 且 HTTP/2/3 的流控制和优先级机制使得高吞吐和低延迟成为可能；
   - SSE 消息只能**文本格式**，Streamable HTTP 支持其他采用更紧凑的编码方式（比如二进制分包、压缩等）。

#### 必须选用以上传输协议吗？

——No，因为无论哪种传输方式，都只是把各种工具的不同接入方式统一起来，对外暴露一种协议的接口而已。

有一些Agent框架，将MCP server定义成局部变量，随时用，随时new, 用完销毁，就不会存在需要更新的问题，也就不需要MCP服务器向MCP客户端发送更新的问题了

![](./img/MCPHost.png)

![img](https://oigi8odzc5w.feishu.cn/space/api/box/stream/download/asynccode/?code=NDllZGE5NmJlN2E3ZjZhNmYzMjY1ZDVkZjE4ZWQ0MDFfNEFsS3o2dkdyY2xmMnBjS2Z1ZEk1UUhqSTVxZW1nckZfVG9rZW46SG5MNWJlNFY3b0s3THJ4cWIzZmNjWm5rbjllXzE3NzM5MDAzMzg6MTc3MzkwMzkzOF9WNA)
...
2. **工具与AI 应用之间的交互必须标准化。**
   ...
   2. AI 应用和工具服务的**接口定义**需要统一（需要提供哪些接口、接口需包含哪些参数）
      ...
      2. 1. **tools/list**（用于返回方法列表） 
         2. **tools/call**（用于执行方法并返回结果。
         3. **notifications/tools/list_changed**（服务端主动推送，用于告知客户端方法更新）[[5\]](https://modelcontextprotocol.io/specification/2025-06-18/server/tools)

      3.   ✅接口参数定义，见文档。以 tools/list 为例： 

      4. ![img](https://oigi8odzc5w.feishu.cn/space/api/box/stream/download/asynccode/?code=YjE0ODQ3NmM0Mzc4NGI0OTYzMDg1YmE2OGJkZTkyZjlfcmh4dmlSdlNxeE9QUk1zeUx5SVF6eXlIMEZNSzJmZlJfVG9rZW46U1A5N2JzRFJzb25aOGV4TlBNT2NVVDMzbjNlXzE3NzM5MDAzMzg6MTc3MzkwMzkzOF9WNA)

      5. ![img](https://oigi8odzc5w.feishu.cn/space/api/box/stream/download/asynccode/?code=MTQ0NDIxMDRiODI2ODRhZDNjN2IyNGJlNzA5ZTVmZjdfMWRvSFJONThKWTA5SWFnUU9IWDFmTjdnV1JYdloxRjZfVG9rZW46R2FRR2JjamVrb3BpeDF4ZFZ3aGN5N1pybk5nXzE3NzM5MDAzMzg6MTc3MzkwMzkzOF9WNA)
   ...
   4. AI 应用接入工具的**配置内容**需要进行标准化定义
      ...
      3. 1.  在 mcpServers 配置中新增一个叫 "amap-maps"（你自己起名）的 MCP Server
         2.  通过 npx -y @amap/amap-maps-mcp-server 命令，运行高德官方提供的 MCP server
...
1. 初始化并获取工具列表
   1. 在对话之前，进行初始化，类似于TCP的三次握手
   2. 初始化之后，MCP client 请求工具列表，MCP server 获取工具列表
2. 用户提问
   1. 用户发起提问
   2. Client 拿着提问+可用工具列表，调用大模型
   3. 大模型返回调用指令
3. 指令执行
   1. client 向MCP server发起工具调用请求(tools/call())
   2. MCP server 接到请求，开始实际调用工具
   3. 返回工具调用的原始方法
   4. MCP server 封装标准化的返回结果，返回AI 应用程序给后端
4. 大模型处理结果
   1. 后端逻辑，带着用户的原始输入、工具调用的结果，输入给大模型进行提问
   2. 大模型返回结果给后端
   3. 后端返回结果给用户

![](./img/MCPFuncCallingProcess.png)

### MCP 与Function calling

> **值得一提的是，<u>MCP 协议和 Function Calling</u>之间绝不是“技术递进”的关系。所谓“MCP 协议会取代 Function Calling”的说法，其实是一种不严谨的表达。**

1. Function calling 主要是 AI 应用程序和大模型交互的这部分，也就是上图中，3.1~3.4的部分

2. MCP 协议，关心的是AI应用程序，怎么和MCP server进行交互的这部分，也就是怎么拿到function calling 过程中所需要的函数描述
