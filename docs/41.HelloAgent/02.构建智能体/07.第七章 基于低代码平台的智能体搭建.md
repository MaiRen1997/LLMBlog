---
title: 第七章 基于低代码平台的智能体搭建
date: 2026-03-11 21:14:55
permalink: /pages/0be7d5/
categories:
  - HelloAgent
  - 构建智能体
tags:
...
对于Memory唯一需要注意的是，这里我们使用每个邮箱的线程名作为唯一标识，能保证存储的唯一性，设置的Key为` $('Gmail').item.json.threadId `(需要两侧加大括号) 
...
- <strong>To</strong>: ` $('Gmail').item.json.From `(需要两侧加大括号) (或其他触发器中的发件人字段)
- <strong>Subject</strong>: `Re:   $('Gmail').item.json.Subject `(需要两侧加大括号) 
- <strong>Message</strong>: ` $json.output `(需要两侧加大括号) 
