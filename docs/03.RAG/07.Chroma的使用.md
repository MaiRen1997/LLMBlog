---
title: Chroma的使用
date: 2026-05-10 10:24:28
permalink: /pages/94c029/
categories:
  - RAG
tags:
  - 
...
1. 切分文档并存储
   1. 加载文档
   2. 文档切块
   3. 使用`向量模型实例.embed_documents`对文档块进行向量化
   4. 递归方式，将向量化后的文档块，通过`Chroma实例.add_documents`添加到向量数据库
2. 查询文档
   1. 调用模型，使用`向量模型实例.embed_query`将用户问题向量化
   2. 使用`Chroma实例.similarity_search_with_score(用户问题向量，需要取前几个)`，进行结果搜索
