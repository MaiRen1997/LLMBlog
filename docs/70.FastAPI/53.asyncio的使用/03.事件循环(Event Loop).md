...
date: 2026-04-05 23:29:16
permalink: /pages/fa5bb1/
categories:
  - FastAPI
  - asyncio的使用
tags:
...
**事件循环就像一个永不停止的循环(死循环)，不断地检查是否有可以执行的任务**。

## 常用的属性API

1. 使用`asyncio.get_event_loop`获取当前事件循环

2. 使用`loop.is_running`判断是否运行

3. 使用`loop.is_closed`判断是否关闭

4. 使用`loop.asyncio.gather`

5. 使用`loop.run_until_complete`运行协程，直到完成，然后返回结果或者抛出异常

   1. python3.7 之前用`run_until_complete`, python3.7之后，用`asyncio.run`
   ...
   2. 如：
      ...
      # 新方式（Python 3.7+，推荐）
      result = asyncio.run(main())  # 内部做了: 创建循环 → run_until_complete → 关闭
      ```
...
# 将任务放到 任务列表
result = loop.run_until_complete(任务)
