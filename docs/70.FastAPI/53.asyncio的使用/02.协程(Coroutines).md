---
title: 协程(Coroutines)
date: 2026-04-05 23:20:31
permalink: /pages/4155d7/
categories:
  - FastAPI
  - asyncio的使用
tags:
...
2. yield关键字：保存状态，切换到其他函数去执行，执行完再切换回来

3. asyncio模块(python3.4)

4. async await 关键字实现(python3.5)

5. 注意：执行协程函数，创建协程对象，函数内部代码不会执行

   1. 如果要运行协程函数内部代码，必须要将协程对象交给`事件循环`、`asyncio.run()进行运行`
      ...
      result = func() # 使用这行代码，不会调用
      result = asyncio.run(func()) # 使用run 函数才会执行
      print(result)
      ```
...
await 后面只能跟可等待的对象(协程对象、Future、Task对象)

1. 协程对象：使用async + await 定义协程对象，然后使用大括号调用，返回的就是协程对象，下面的resultExe就是协程对象
...
1. 使用`@asyncio.coroutine`装饰器，构建成协程函数
...
def func1():
    print(1)
    yield from asyncio.sleep(2) # 遇到I/O 耗时操作，自动化切换到tasks中的其他任务
    print(2)
