---
title: Task对象
date: 2026-04-06 10:02:04
permalink: /pages/041436/
categories:
  - FastAPI
  - asyncio的使用
tags:
...
Task用来并发的调度当前的协程，通过`asyncio.crate_task(协程对象)`**(python3.7之后)**的方式创建Task对象，这样可以让协程加入事件循环中等待被调度执行

除了使用asyncio.create_task()，还可以用低层级的loop.create_task()或ensure_future()函数，不建议手动实例化Task对象
